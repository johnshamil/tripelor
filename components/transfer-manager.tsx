...
const blankSchedule = (): ScheduleForm => ({ operator: "", route: "", dayOfWeek: 0, departureTime: "10:00", active: true, capacity: 20, pricePerPerson: 50, notes: "" });
...
export default function TransferManager() {
  ...
  return <main className="container py-8 pb-28 md:py-14">
    ...
    <section className="mt-8 rounded-2xl border border-gold/20 bg-gold/[.04] p-5 md:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[.22em] text-gold">Departure schedule</p><h2 className="mt-2 text-2xl font-semibold">Published transfer routes</h2><p className="mt-1 text-sm text-gray-400">Add an island route, operating day, time, operator and fare. Published routes appear on the customer transfer page. Add another departure for each operating day.</p></div><button type="button" onClick={() => setScheduleForm(blankSchedule())} className="btn-gold gap-2"><Plus className="h-4 w-4" /> Add transfer route</button></div>
...
function ManualTransfer({ onCreated }: { onCreated: (request: TransferRequest) => void }) {
  ...
  return <section className="mt-6 rounded-2xl border border-gold/25 bg-gold/[.04] p-5">
    <div className="flex flex-wrap items-center justify-between gap-4">
      <div><h2 className="text-xl font-semibold">Customer transfers</h2><p className="mt-2 text-sm text-gray-400">Add a customer transfer for any island, including return journeys.</p></div>
      <button type="button" className="btn-gold" disabled={busy} onClick={() => setOpen(!open)}>{open ? "Close form" : "Add customer request"}</button>
    </div>
