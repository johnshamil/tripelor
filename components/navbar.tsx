    ...
    <header data-menu-open={open || undefined} className="sticky top-0 z-50 border-b border-white/10 bg-[#041117]/95 backdrop-blur-xl">
      <div className="container flex h-[64px] items-center justify-between md:h-[76px]">
        ...
        <nav className="hidden items-center gap-4 xl:gap-6 xl:flex" aria-label="Main navigation">
          ...
          <Link href="/maldives-matchmaker" className="btn-gold min-h-[44px] px-5 py-2">
            {copy.helpMeChoose}
          </Link>
        </nav>
      ...
      {!adminSurface && <Link href="/maldives-matchmaker" onClick={() => setOpen(false)} className="flex min-h-[44px] items-center justify-center gap-2 border-t border-white/10 px-4 text-xs text-[#ead7aa] xl:hidden">{copy.unsure} <span className="font-semibold underline">{copy.helpMeChoose}</span></Link>}
