...
export default function Navbar() {
  ...
  const links = [["/", "Home"], ["/stays/uhoos-lavish-oasis", "Stays"], ["/island-adventures", "Packages"], ["/experience-bundles", "Bundles"], ["/tours", "Experiences"], ["/speedboat", "Speedboat"], ["/reviews", "Reviews"], ["/contact", "Contact"]];
  ...
  return <header className="sticky top-0 z-50 border-b border-white/10 bg-black/90 backdrop-blur-xl">
    <div className="container flex h-16 items-center justify-between">
      ...
      <nav className="hidden items-center gap-4 lg:flex">{links.map(([href,label]) => <Link key={href} href={href} className="nav-tab text-sm">{label}</Link>)}<Link href="/account" className="nav-tab gap-1.5 text-sm"><UserRound className="h-4 w-4"/>Account</Link><Link href="/build-your-trip" className="btn-outline px-4 py-2.5">Build Your Trip</Link><Link href="/booking?property=Uhoo%27s%20Lavish%20Oasis&mealPlan=Bed%20%26%20Breakfast" className="btn-gold px-4 py-2.5">Book Now</Link></nav>
