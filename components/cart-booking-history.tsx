...
export default function CartBookingHistory({ admin = false }: { admin?: boolean }) {
  ...
  return <section className="container py-10 pb-28 md:py-16">
    ...
    <div className="mt-5 flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm uppercase tracking-[.2em] text-gold">Packages & excursions</p><h1 className="font-display mt-3 text-4xl">{admin ? "Trip planning requests" : "My trip requests"}</h1></div><button type="button" onClick={() => setReload(value => value + 1)} disabled={loading} className="btn-outline min-h-11">Refresh</button></div>
    <p className="mt-4 max-w-3xl leading-7 text-gray-300">{admin ? "Review each customer’s combined selection, dates and contact details. Requests need availability and payment confirmation before a booking is final." : "Your chosen stays and experiences, together in one trip plan. Tripelor will contact you to confirm availability and payment details."}</p>
    {loading ? <p role="status" className="py-10 text-gray-300">Loading trip requests…</p> : error ? <p role="alert" className="mt-8 rounded-xl border border-red-400/30 p-5 text-red-200">{error}</p> : requests.length === 0 ? <div className="card mt-8 p-8"><h2 className="text-2xl">No trip requests yet</h2>{!admin && <Link href="/island-adventures" className="btn-gold mt-5">Explore packages & excursions</Link>}</div> : <div className="mt-8 space-y-6">{requests.map(request => <article key={request.id} className="card min-w-0 p-5 sm:p-7">
      ...
      <p className="mt-5 border-t border-white/10 pt-4 text-right text-lg">Trip estimate: <strong className="text-gold">USD {Number(request.estimated_total).toLocaleString("en-US")}</strong></p>
    </article>)}</div>}
