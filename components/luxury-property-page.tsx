...
export default function LuxuryPropertyPage({eyebrow,name,location,description,photos,startingFrom,rateNote="",bookingHref,highlights,rates,rooms=[],terms=[],videos=[]}:{eyebrow:string;name:string;location:string;description:string;photos:string[];startingFrom:number;rateNote?:string;bookingHref:string;highlights:Highlight[];rates:Rate[];rooms?:Room[];terms?:Term[];videos?:string[]}){
...
{rates.length>0&&<section className="bg-[#06151c]"><div className="container grid gap-10 py-16 md:py-20 lg:grid-cols-[1fr_340px]"><div><p className="eyebrow">{rooms.some(room=>room.mealPlans?.length) ? "Seasonal rates" : "Meal plans"}</p><h2 className="section-title mt-4">Choose how you would like to stay.</h2><div className="mt-8 grid gap-4 md:grid-cols-2">{rates.map(rate=><Link key={`${rate.name}-${rate.price}`} href={rate.bookingHref} className="group border border-white/10 bg-white/[.025] p-6 transition hover:border-[#c9a86a]/45"><Utensils className="h-5 w-5 text-[#c9a86a]"/><h3 className="font-display mt-6 text-2xl text-white">{rate.name}</h3><p className="mt-2 text-sm leading-6 text-white/45">{rate.detail}</p><div className="mt-5 border-t border-white/10 pt-4"><span className="font-display text-3xl text-[#d9bd7b]">USD {rate.price}</span><span className="text-xs text-white/35"> / night</span></div></Link>)}</div></div><aside className="h-fit border border-[#c9a86a]/30 bg-[#0a222b] p-7 lg:sticky lg:top-24"><p className="eyebrow">Ready to book?</p><h3 className="font-display mt-3 text-3xl text-white">{name}</h3><p className="mt-2 flex items-center gap-2 text-sm text-white/45"><MapPin className="h-4 w-4 text-[#c9a86a]"/>{location}</p><div className="mt-6 border-y border-white/10 py-5"><p className="text-[10px] uppercase tracking-[.2em] text-white/35">Rates from</p><p className="font-display mt-2 text-4xl text-[#d9bd7b]">USD {startingFrom}</p>{rateNote&&<p className="mt-2 text-[10px] leading-4 text-white/40">{rateNote}</p>}</div>{rooms.length>0?<a href="#choose-room" className="btn-gold mt-6 w-full justify-center">Choose Your Room</a>:<Link href={bookingHref} className="btn-gold mt-6 w-full justify-center">Check Dates & Book</Link>}<a href={`https://wa.me/9609429403?text=${conciergeMessage}`} target="_blank" rel="noreferrer" className="btn-outline mt-3 w-full justify-center">Ask a Concierge</a></aside></div></section>}
