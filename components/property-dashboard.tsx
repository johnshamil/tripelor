...
function PropertyForm({ form, update, updateRoom, updateSeasonalRate, updateInventoryRule, setForm, openSections, toggle, onUpload, onHostBusy, uploading, saving, onSave, onCancel, notice, error }: { form: FormState; update: <K extends keyof FormState>(key: K, value: FormState[K]) => void; updateRoom: (index: number, key: keyof FormRoom, value: string | number) => void; updateSeasonalRate: (index: number, key: keyof SeasonalRate, value: string | number) => void; updateInventoryRule: (index: number, key: keyof InventoryRule, value: string | number | boolean) => void; setForm: React.Dispatch<React.SetStateAction<FormState | null>>; openSections: Record<string, boolean>; toggle: (section: string) => void; onUpload: (files: FileList | null, target: PhotoTarget) => void; onHostBusy: (busy: boolean) => void; uploading: boolean; saving: boolean; onSave: () => void; onCancel: () => void; notice: string; error: string }) {
  ...
  if (preview) {
    ...
    return <section className="property-editor mt-6">
      <div className="property-editor-actions sticky top-[65px] z-40 flex items-center justify-between gap-3 rounded-xl border border-gold/30 bg-[#071922] p-3 md:top-[77px] sm:p-4">
        <div className="min-w-0"><h2 className="text-sm font-semibold text-gold sm:text-base">Private property preview</h2><p className="mt-1 hidden text-xs text-gray-400 sm:block">Includes your unsaved edits. Nothing has been published. Booking is disabled here.</p></div>
        <button type="button" className="btn-gold shrink-0 px-3 normal-case tracking-normal" onClick={() => { setPreview(false); requestAnimationFrame(() => window.scrollTo({ top: previewScroll })); }}>Back to editing</button>
      </div>
  ...
  return <section className="property-editor mt-6 min-w-0 max-w-6xl">
    <div className="flex items-start justify-between gap-3 rounded-2xl border border-gold/20 bg-gold/[.05] p-4 sm:p-6">
      <div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-[.22em] text-gold">{form.id ? "Edit property" : "New property"}</p><h2 className="mt-2 break-words text-2xl font-semibold">{form.name || "Untitled property"}</h2><p className="mt-2 text-sm leading-6 text-gray-400">Preview your changes, then save when ready.</p></div>
      <button type="button" onClick={onCancel} disabled={saving || uploading} className="flex min-h-12 shrink-0 items-center gap-1 rounded-xl px-2 text-sm text-gray-300 disabled:opacity-50"><X className="h-4 w-4" aria-hidden="true"/> Cancel</button>
    </div>
    <div className="property-editor-actions sticky top-[65px] z-40 mt-3 grid grid-cols-2 gap-2 rounded-2xl border border-gold/25 bg-[#071922] p-2 shadow-lg md:top-[77px]">
      <button type="button" disabled={saving || uploading} className="btn-outline px-2 normal-case tracking-normal disabled:opacity-50" onClick={() => { setPreviewScroll(window.scrollY); setPreview(true); window.scrollTo({ top: 0 }); }}>Preview Property</button>
      <button type="button" onClick={onSave} disabled={saving || uploading} className="btn-gold gap-2 px-2 normal-case tracking-normal disabled:opacity-50">{saving ? <Loader2 className="h-4 w-4 shrink-0 animate-spin"/> : <Save className="h-4 w-4 shrink-0"/>}{saving ? "Saving…" : uploading ? "Uploading…" : form.status === "published" ? "Save & Publish" : "Save Draft"}</button>
    </div>
    <label className="mt-5 grid min-w-0 gap-2 text-sm font-medium">Go to section
      <select value="" className="min-h-12 w-full min-w-0 rounded-xl border border-white/20 bg-[#071922] p-3 text-base" onChange={event => {
        const section = event.target.value;
        if (["rooms", "seasonal", "commercial", "partner"].includes(section) && !openSections[section]) toggle(section);
        requestAnimationFrame(() => document.getElementById("property-" + section)?.scrollIntoView({ block: "start", behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth" }));
      }}>
        <option value="">Choose what to edit…</option>
        <option value="details">Property details</option><option value="rooms">Rooms & rates</option><option value="photos">Property photographs</option><option value="host">Host welcome</option><option value="experiences">Things to do</option><option value="seasonal">Seasonal rates</option><option value="know">Know Before You Book</option><option value="arrival">Arrival guide</option><option value="commercial">Booking conditions</option><option value="partner">Partner contacts</option><option value="publishing">Publishing</option>
      </select>
    </label>
    ...
    <div className="mt-5 grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,.9fr)]">
      <div className="min-w-0 space-y-5">
        <Panel id="property-details" title="Property details" subtitle="The information guests will see." open={true} onToggle={() => {}} collapsible={false}>
  ...
  <div className="mt-4 flex flex-wrap gap-2">{[["BB","Bed & Breakfast"],["HB","Half Board"],["FB","Full Board"],["RO","Room Only"],["AI","All Inclusive"]].filter(([,name])=>!room.mealRates.some(rate=>rate.mealPlan.toLowerCase()===name.toLowerCase())).map(([label,name])=><button key={label} type="button" onClick={()=>addMealRate(index,name)} className="min-h-11 rounded-lg border border-gold/30 px-3 py-2 text-xs text-gold">+ {label}</button>)}<button type="button" onClick={()=>addMealRate(index,"")} className="min-h-11 rounded-lg border border-white/20 px-3 py-2 text-xs">+ Other plan</button></div>
  <p className="mt-3 text-xs text-gray-500">Selling prices are shown to guests. Contracted rates stay private.</p>
        ...
        <Panel id="property-experiences" title={`Things to do near this stay · ${(form.experiences || []).length}`} subtitle="Add local experiences for this property. Prices are starting estimates; customers save their interest to My Tripelor." open={true} onToggle={()=>{}} collapsible={false}>
          <div className="min-w-0 space-y-5">{(form.experiences || []).map(experience=><div key={experience.id} className="rounded-xl border border-gold/20 p-4">
      ...
      <div className="min-w-0 space-y-5">
        ...
        <Panel id="property-commercial" title="Booking conditions" subtitle="Shown on the public property page." open={openSections.commercial} onToggle={() => toggle("commercial")}>
          ...
          <TextArea label="Transfer options" value={form.transfers} onChange={v => update("transfers", v)} placeholder="e.g. Speedboat, public ferry, airport transfer details."/>
          <TextArea label="Cancellation conditions" value={form.cancellation} onChange={v => update("cancellation", v)} placeholder="Explain notice periods and non-refundable amounts."/>
          <TextArea label="Payment conditions" value={form.payment} onChange={v => update("payment", v)} placeholder="e.g. Payment is due after Tripelor confirms availability."/>
        </Panel>
        <Panel id="property-partner" title="Partner contact & commercial terms" subtitle="Only visible to your admin account." open={openSections.partner} onToggle={() => toggle("partner")}><Field label="Partner / property manager" value={form.partnerName} onChange={v => update("partnerName", v)} placeholder="Manager name"/><Field label="Partner email" value={form.partnerEmail} onChange={v => update("partnerEmail", v)} placeholder="manager@example.com" type="email"/><Field label="Partner phone / WhatsApp" value={form.partnerPhone} onChange={v => update("partnerPhone", v)} placeholder="+960 ..."/><p className="mt-3 text-xs leading-5 text-gray-500">Contracted rates, partner contacts and unpublished drafts are never sent to public visitors.</p></Panel>
        <section id="property-publishing" className="rounded-2xl border border-white/10 bg-white/[.025] p-5"><p className="text-xs font-semibold uppercase tracking-[.18em] text-gold">Publishing</p><div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={() => update("status", "draft")} className={`rounded-xl border px-3 py-3 text-sm font-semibold ${form.status === "draft" ? "border-gold bg-gold/15 text-gold" : "border-white/10 text-gray-500"}`}>Save as draft</button><button type="button" onClick={() => update("status", "published")} className={`rounded-xl border px-3 py-3 text-sm font-semibold ${form.status === "published" ? "border-emerald-400/50 bg-emerald-400/10 text-emerald-200" : "border-white/10 text-gray-500"}`}>Publish live</button></div><p className="mt-3 text-xs leading-5 text-gray-500">Publishing makes the property visible on the Stays page and its public property link.</p></section>
      </div>
    </div>
  </section>;
}
...
function Panel({ id, title, subtitle, open, onToggle, children, collapsible = true }: { id?: string; title: string; subtitle: string; open: boolean; onToggle: () => void; children: React.ReactNode; collapsible?: boolean }) { return <section id={id} className="min-w-0 rounded-2xl border border-white/10 bg-white/[.025] p-4 sm:p-5 md:p-6"><button type="button" onClick={onToggle} disabled={!collapsible} className="flex min-h-12 w-full items-start justify-between gap-4 text-left disabled:cursor-default" aria-expanded={collapsible ? open : undefined}><span><h3 className="text-lg font-semibold">{title}</h3><p className="mt-1 text-xs text-gray-500">{subtitle}</p></span>{collapsible && (open ? <ChevronUp className="mt-1 h-5 w-5 text-gold" /> : <ChevronDown className="mt-1 h-5 w-5 text-gray-500" />)}</button>{open && <div className="mt-5">{children}</div>}</section>; }
function Field({ label, value, onChange, placeholder, hint, type = "text" }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; hint?: string; type?: string }) { return <label className="grid min-w-0 gap-2 text-sm"><span className="font-medium text-white/85">{label}</span><input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="rounded-xl border border-white/10 bg-black/30 px-4 py-3 outline-none transition focus:border-gold" />{hint && <span className="text-[11px] text-gray-500">{hint}</span>}</label>; }
function NumberField({ label, value, onChange, min, max, hint }: { label: string; value: number; onChange: (value: number) => void; min?: number; max?: number; hint?: string }) { return <label className="grid min-w-0 gap-2 text-sm"><span className="font-medium text-white/85">{label}</span><input type="number" min={min} max={max} step="0.01" value={value} onChange={e => onChange(Number(e.target.value))} className="rounded-xl border border-white/10 bg-black/30 px-4 py-3 outline-none transition focus:border-gold" />{hint && <span className="text-[11px] text-gray-500">{hint}</span>}</label>; }
function TextArea({ label, value, onChange, placeholder, hint }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; hint?: string }) { return <label className="mt-4 grid min-w-0 gap-2 text-sm"><span className="font-medium text-white/85">{label}</span><textarea rows={3} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="rounded-xl border border-white/10 bg-black/30 px-4 py-3 leading-6 outline-none transition focus:border-gold" />{hint && <span className="text-[11px] text-gray-500">{hint}</span>}</label>; }
