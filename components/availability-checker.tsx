...
export default function AvailabilityChecker({
  roomType,
  propertyName = "Uhoo's Lavish Oasis",
  displayName,
}: Props) {
  ...
  async function check() {
    ...
    setOk(null);
    if (!checkIn || !checkOut) {
      setStatus("Choose check-in and check-out dates.");
      return;
    }
    if (checkOut <= checkIn) {
    ...
    try {
      ...
      setStatus(
        available
          ...
          : `${roomLabel} is not available for these dates. Please try different dates.`,
      );
    } catch (e) {
      setStatus(e instanceof Error ? e.message : "Unable to check availability.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="card p-6 md:p-7">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-xs uppercase tracking-[.22em] text-gold">Live availability</p>
          <h3 className="text-xl font-bold">Check {roomLabel}</h3>
        </div>
      </div>
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <label className="grid gap-2 text-sm">
          <span className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-gold" />Check-in</span>
          <input type="date" value={checkIn} onChange={(e) => setCheckIn(e.target.value)} className="rounded-xl border border-white/10 bg-black px-4 py-3" />
        </label>
        <label className="grid gap-2 text-sm">
          <span className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-gold" />Check-out</span>
          <input type="date" value={checkOut} onChange={(e) => setCheckOut(e.target.value)} className="rounded-xl border border-white/10 bg-black px-4 py-3" />
        </label>
      </div>
