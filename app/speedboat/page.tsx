...
type Schedule = { id: number; route: string; day_of_week: number; departure_time: string; operator: string; capacity?: number; price_per_person?: number };
...
export default function SpeedboatPage() {
  const [route, setRoute] = useState(""); const [departureId, setDepartureId] = useState("");
  ...
  const routes = useMemo(() => Array.from(new Set(schedule.map(item => item.route))).sort(), [schedule]);
  const departures = useMemo(() => schedule.filter(item => item.route === route && item.day_of_week === new Date(date + "T12:00:00Z").getUTCDay() && (!time || toMinutes(item.departure_time) >= toMinutes(time))).sort((a,b) => toMinutes(a.departure_time) - toMinutes(b.departure_time)), [schedule, route, date, time]);
  const recommendation = departures.find(item => String(item.id) === departureId);
  const selectedPrice = Number(recommendation?.price_per_person || 0);
  ...
  async function submit(event: FormEvent) {
    ...
    if (!recommendation) { setStatus("Please select a route and available departure."); return; }
    setSending(true);
    try {
      const response = await fetch("/api/speedboat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ scheduleId: recommendation.id, arrivalDate: date, arrivalTime: time, flightNumber: flight, recommendedDeparture: recommendation && recommendation.departure_time || null, seats: Number(seats), pricePerPerson: selectedPrice, total: Number(seats) * selectedPrice, fullName: name, email, phone, notes }) });
  ...
  return <section className="container py-16 md:py-20"><div className="mx-auto max-w-4xl"><p className="text-sm uppercase tracking-[.3em] text-gold">Island transfers</p><h1 className="mt-2 text-4xl font-bold md:text-6xl">Choose your island transfer</h1><p className="mt-4 text-gray-400">Choose a route published by Tripelor, enter your travel date and ready time, then select a departure. All times are Maldives local time (UTC+5).</p>
    <div className="card mt-8 p-6"><label className="grid gap-2">Transfer route<select value={route} onChange={event => { setRoute(event.target.value); setDepartureId(""); }} className="rounded-xl border border-white/10 bg-black px-4 py-3"><option value="">Select a route</option>{routes.map(value => <option key={value} value={value}>{value}</option>)}</select></label>{!routes.length && <p className="mt-3 text-gray-400">No published routes are available right now. Please try again later or contact Tripelor.</p>}</div>
    <div className="card mt-8 p-6"><div className="flex items-center gap-2 text-gold"><Plane className="h-5 w-5" /> <b>Your flight</b></div><div className="mt-5 grid gap-3 md:grid-cols-[1.2fr_auto]"><input value={flight} onChange={(event) => { setFlight(event.target.value.toUpperCase()); setFlightInfo(null); setLookupStatus(""); }} placeholder="Flight number (e.g. EK652)" className="rounded-xl border border-white/10 bg-black px-4 py-3"/><button type="button" onClick={lookupFlight} disabled={lookup} className="btn-gold gap-2"><Search className="h-4 w-4" />{lookup ? "Looking up..." : "Find My Flight"}</button></div><p className="mt-2 text-xs text-gray-500">Optional for airport arrivals: use flight lookup, then allow at least 90 minutes after landing when setting your ready time.</p>{lookupStatus && <div className={`mt-4 rounded-xl border p-4 text-sm ${flightInfo ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-200" : "border-amber-500/30 bg-amber-500/10 text-amber-100"}`}>{lookupStatus}</div>}{flightInfo && <div className="mt-4 rounded-2xl border border-white/10 bg-white/[.03] p-5"><div className="flex items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[.2em] text-gold">Flight found</p><h2 className="mt-1 text-2xl font-bold">{flightInfo.airline ? `${flightInfo.airline} · ` : ""}{flightInfo.flightNumber}</h2><p className="mt-1 text-sm text-gray-400">{flightInfo.origin || "Origin"} → {flightInfo.arrivalAirport || "Velana International Airport"}</p></div><CheckCircle2 className="h-6 w-6 text-emerald-400" /></div><div className="mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-4"><Info label="Arrival date" value={flightInfo.arrivalDate} /><Info label="Arrival time" value={niceTime(flightInfo.arrivalTime)} /><Info label="Terminal" value={flightInfo.terminal || "TBC"} /><Info label="Status" value={flightInfo.status || "Scheduled"} /></div></div>}<div className="mt-5 grid gap-4 md:grid-cols-2"><label className="grid gap-2 text-sm">Travel date<input required type="date" value={date} onChange={(event) => setDate(event.target.value)} className="rounded-xl border border-white/10 bg-black px-4 py-3" /></label><label className="grid gap-2 text-sm">Ready time at departure point<input required type="time" value={time} onChange={(event) => setTime(event.target.value)} className="rounded-xl border border-white/10 bg-black px-4 py-3" /></label></div>{route && date && <label className="mt-5 grid gap-2">Available departures<select value={recommendation ? departureId : ""} onChange={event => setDepartureId(event.target.value)} className="rounded-xl border border-white/10 bg-black px-4 py-3"><option value="">Select a departure</option>{departures.map(item => <option key={item.id} value={item.id}>{niceTime(item.departure_time)} · {item.operator} · USD {Number(item.price_per_person || 0).toFixed(2)} per person</option>)}</select>{!departures.length && <span className="text-amber-200">No departures for this route and date after your ready time. Choose another date or contact Tripelor.</span>}</label>}</div>
    <form onSubmit={submit} className="card mt-6 grid gap-5 p-6"><div className="grid gap-4 md:grid-cols-2"><div className="rounded-xl border border-gold/30 bg-gold/10 p-4"><b className="text-gold">{recommendation ? "USD " + selectedPrice.toFixed(2) + " per person" : "Select a departure to see the fare"}</b><p className="mt-1 text-sm text-gray-300">{recommendation ? recommendation.operator + " · Total USD " + (Number(seats) * selectedPrice).toFixed(2) : "Payment arrangements are confirmed by Tripelor."}</p></div><div className="rounded-xl border border-gold/30 bg-gold/10 p-4"><b className="text-gold">Book at least 24 hours ahead</b><p className="mt-1 text-sm text-gray-300">Transfer schedule and seats are subject to confirmation.</p></div></div><label className="grid gap-2 text-sm"><span className="flex gap-2"><Users className="text-gold" />Passengers / seats</span><select value={seats} onChange={(event) => setSeats(event.target.value)} className="rounded-xl border border-white/10 bg-black px-4 py-3">{Array.from({ length: 20 }, (_, index) => index + 1).map((number) => <option key={number}>{number}</option>)}</select></label><div className="grid gap-4 md:grid-cols-3"><input required value={name} onChange={(event) => setName(event.target.value)} placeholder="Full name" className="rounded-xl border border-white/10 bg-black px-4 py-3" /><input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Email" className="rounded-xl border border-white/10 bg-black px-4 py-3" /><input required value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="Phone / WhatsApp" className="rounded-xl border border-white/10 bg-black px-4 py-3" /></div><textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Transfer notes" className="rounded-xl border border-white/10 bg-black px-4 py-3" />{status && <div className="rounded-xl bg-white/5 p-4 text-sm">{status}</div>}<button disabled={sending || !recommendation} className="btn-gold w-full md:w-auto"><Mail className="h-5 w-5" />{sending ? "Sending..." : "Request selected transfer"}</button><p className="text-xs text-gray-500">Flight times come from an external aviation-data provider and can change. Final departure and seat availability still require confirmation.</p></form></div></section>;
}
