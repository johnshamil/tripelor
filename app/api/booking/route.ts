...
export async function POST(request: Request) {
  try {
    ...
    const {
      propertyName, fullName, email, phone, destination, checkIn, checkOut, nights,
      adults, children, roomType, rooms, mealPlan, nightlyRate, estimatedTotal, specialRequests,
    } = body;
    ...
    const subject = `New Tripelor booking request - ${escapeHtml(propertyName || destination)} - ${escapeHtml(fullName)}`;
    const html = `
        ...
        <div style="border:1px solid #e5e5e5;border-top:0;padding:26px;border-radius:0 0 14px 14px">
          ...
          <table style="width:100%;border-collapse:collapse">
            <tbody>
              ...
              ${estimatedTotal ? `<tr><td style="padding:8px 0;font-weight:bold">Estimated Room Total</td><td style="font-weight:bold">USD ${escapeHtml(estimatedTotal)}</td></tr>` : ""}
              <tr><td style="padding:8px 0;font-weight:bold">Special Request</td><td>${escapeHtml(specialRequests || "None")}</td></tr>
            </tbody>
          </table>
          <p style="margin-top:24px;color:#666;font-size:13px">Displayed totals are estimates. Availability and final price should be confirmed before booking.</p>
        </div>
      </div>`;
    ...
    if (!resendResponse.ok) {
      ...
      return Response.json({ error: result?.message || "Unable to send booking email." }, { status: 500 });
    }
