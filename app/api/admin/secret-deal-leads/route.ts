...
function errorResponse(error: unknown) {
  const message = error instanceof Error ? error.message : "Unable to load Secret Deal leads.";
...
export async function GET() {
  try {
    ...
    const response = await fetch(
      `${url}/rest/v1/secret_deal_leads?select=*&order=created_at.desc&limit=250`,
      { headers: { apikey: key, Authorization: `Bearer ${key}` }, cache: "no-store" },
    );
    const raw = await response.text();
    const data = raw ? JSON.parse(raw) : [];
    if (!response.ok) throw new Error(data?.message || "Unable to load Secret Deal leads.");
...
export async function PATCH(request: Request) {
  try {
    ...
    const { url, key } = cfg();
    const response = await fetch(`${url}/rest/v1/secret_deal_leads?id=eq.${encodeURIComponent(id)}`, {
      method: "PATCH",
      headers: {
        apikey: key,
