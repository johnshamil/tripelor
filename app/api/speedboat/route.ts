...
async function scheduleFor(arrivalDate: string, recommendedDeparture: string) {
  if (!recommendedDeparture) return null;
  try {
    const { url, key } = propertyConfig();
    const dayOfWeek = new Date(`${arrivalDate}T12:00:00Z`).getUTCDay();
    const response = await fetch(`${url}/rest/v1/speedboat_schedule?day_of_week=eq.${dayOfWeek}&active=eq.true&route=eq.Male%20to%20Felidhoo&select=operator,price_per_person,capacity,departure_time`, { headers: { apikey: key, Authorization: `Bearer ${key}` }, cache: "no-store" });
    if (!response.ok) return null;
    const rows = await response.json();
    return rows.find((row: any) => String(row.departure_time || "").slice(0, 5) === recommendedDeparture.slice(0, 5)) || null;
  } catch {
    return null;
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const recommendedDeparture = cleanText(body.recommendedDeparture || "", 8);
    ...
    const matchedSchedule = await scheduleFor(arrivalDate, recommendedDeparture);
    ...
    const rows = `<tr><td style="padding:8px 0;font-weight:bold">Request reference</td><td>${escapeHtml(requestReference)}</td></tr><tr><td style="padding:8px 0;font-weight:bold">Arrival date</td><td>${escapeHtml(arrivalDate)}</td></tr><tr><td style="padding:8px 0;font-weight:bold">Arrival time</td><td>${escapeHtml(arrivalTime)}</td></tr><tr><td style="padding:8px 0;font-weight:bold">Flight</td><td>${escapeHtml(flightNumber || "Not provided")}</td></tr><tr><td style="padding:8px 0;font-weight:bold">Seats</td><td>${escapeHtml(seats)}</td></tr><tr><td style="padding:8px 0;font-weight:bold">Fare</td><td>USD ${escapeHtml(pricePerPerson)} per person</td></tr><tr><td style="padding:8px 0;font-weight:bold">Total</td><td style="font-weight:bold">USD ${escapeHtml(total)}</td></tr><tr><td style="padding:8px 0;font-weight:bold">Payment</td><td>Pay directly to the boat operator or Uhoo's Lavish Oasis</td></tr>`;
