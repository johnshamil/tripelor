...
export default function BookingPage() {
  ...
  const estimatedTotal = packageName && packagePrice ? packagePrice : (nightlyRate && nights > 0 ? nightlyRate * nights * Number(rooms) : 0);
  ...
  return <section className="container py-16"><div className="mx-auto max-w-5xl">
    ...
    <form className="card mt-10 grid gap-6 p-6 md:p-8" onSubmit={e => e.preventDefault()}>
      ...
      {!packageName && <div className="rounded-xl border border-gold/30 bg-gold/10 px-4 py-3 text-sm text-gold">{propertyName} · BB ${PROPERTY_RATES[propertyName]["Bed & Breakfast"]} · HB ${PROPERTY_RATES[propertyName]["Half Board"]} · FB ${PROPERTY_RATES[propertyName]["Full Board"]} per room/night</div>}
      ...
      {packageName && <p className="-mt-3 text-xs text-gray-500">For package bookings, choosing your check-in date automatically sets check-out 5 nights later. You can still adjust it if needed.</p>}
      {nights > 0 && <div className="rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm">Stay length: <span className="text-gold">{nights} night{nights === 1 ? "" : "s"}</span>{estimatedTotal > 0 && <> · {packageName ? "Package price" : "Estimated room total"}: <span className="font-semibold text-gold">USD {estimatedTotal}</span></>}</div>}
      ...
      <div className="grid gap-5 md:grid-cols-2"><label className="grid gap-2"><span className="flex items-center gap-2 text-sm font-medium"><BedDouble className="h-4 w-4 text-gold"/>Room type</span><select value={roomType} onChange={e => setRoomType(e.target.value)} className="rounded-xl border border-white/10 bg-black px-4 py-3"><option>Deluxe Room</option></select></label><label className="grid gap-2"><span className="flex items-center gap-2 text-sm font-medium"><BedDouble className="h-4 w-4 text-gold"/>Number of rooms</span><select value={rooms} onChange={e => setRooms(e.target.value)} className="rounded-xl border border-white/10 bg-black px-4 py-3">{[1,2,3,4].map(n => <option key={n} value={n}>{n} Room{n > 1 ? "s" : ""}</option>)}</select></label></div>
      ...
      {!packageName && <p className="-mt-3 text-sm text-gray-400">Selected rate: <span className="font-semibold text-gold">USD {nightlyRate} per room/night</span></p>}
      ...
      <textarea value={specialRequests} onChange={e => setSpecialRequests(e.target.value)} rows={4} placeholder="Special requests, speedboat transfer, honeymoon setup, excursions, etc." className="rounded-xl border border-white/10 bg-black px-4 py-3"/>
      {status && <div className="rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-gray-200">{status}</div>}
      <button type="button" onClick={sendBookingEmail} disabled={sending} className="btn-gold w-full gap-2 disabled:opacity-60 md:w-auto"><Mail className="h-5 w-5"/>{sending ? "Sending..." : packageName ? "Send Package Booking Request" : "Send Booking Request"}</button>
