...
import { ArrowLeft, Mail, MessageCircle, Search, Sparkles } from "lucide-react";
...
type Lead = {
  ...
  recommended_property: string;
  recommended_room: string;
  estimated_stay_total: number;
...
export default function MatchmakerLeadsPage() {
  ...
  async function load() {
    const response = await fetch("/api/admin/matchmaker-leads", { cache: "no-store" });
    const data = await response.json();
    if (response.status === 401) {
      window.location.href = "/login?next=%2Fadmin%2Fmatchmaker-leads";
      return;
    }
    if (!response.ok) throw new Error(data.error || "Unable to load Matchmaker leads.");
    setLeads(data.leads || []);
  }
  ...
  async function setStatus(id: string, status: Lead["status"]) {
    ...
    setError("");
    try {
      const response = await fetch("/api/admin/matchmaker-leads", {
        method: "PATCH",
  ...
  const visible = useMemo(() => {
    ...
    return leads.filter(lead => !search || [
      ...
      lead.recommended_room,
      lead.destination,
      lead.travel_style,
    ].join(" ").toLowerCase().includes(search));
  }, [leads, query]);

  if (loading) return <main className="container py-20 text-gray-400">Loading Matchmaker leads...</main>;
  ...
  return (
    <main className="container py-8 pb-24 md:py-14">
      ...
      <div className="mt-5 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="eyebrow flex items-center gap-2"><Sparkles className="h-4 w-4" /> Customer acquisition</p>
          <h1 className="mt-3 text-3xl font-semibold md:text-5xl">Maldives Matchmaker Leads</h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-gray-400">Follow up visitors who asked Tripelor to match their Maldives stay.</p>
        </div>
      ...
      <div className="relative mt-7">
        ...
        <input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search name, email, WhatsApp, property or reference" className="min-h-12 w-full rounded-xl border border-white/10 bg-black/25 pl-11 pr-4 text-sm outline-none focus:border-gold/40" />
      </div>

      <section className="mt-6 grid gap-4">
        {visible.length === 0 ? (
          <div className="rounded-2xl border border-white/10 bg-white/[.02] p-10 text-center text-gray-500">No Matchmaker leads yet.</div>
        ) : visible.map(lead => {
          const whatsappHref = lead.whatsapp
            ? `https://wa.me/${lead.whatsapp.replace(/\D/g, "")}?text=${encodeURIComponent(`Hello ${lead.name}, this is Tripelor regarding your Maldives Matchmaker request ${lead.reference}.`)}`
            : "";
          return (
              ...
              <div className="grid gap-5 lg:grid-cols-[1fr_auto]">
                <div>
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    {lead.marketing_consent && <span className="rounded-full border border-emerald-500/20 bg-emerald-500/5 px-3 py-1 text-[10px] text-emerald-300">Marketing opt-in</span>}
                  </div>
                  ...
                  <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <Info label="Arrival" value={lead.arrival || "Flexible"} />
                    <Info label="Budget" value={Number(lead.budget_usd) ? `USD ${Number(lead.budget_usd).toFixed(2)}` : "Flexible"} />
                    <Info label="Style" value={`${lead.travel_style} · ${lead.destination} · ${lead.meal_preference}`} />
                  </div>

                  <div className="mt-4 rounded-xl border border-gold/15 bg-gold/[.04] p-4">
                    <p className="text-[9px] uppercase tracking-[.16em] text-gray-500">Automatic top match</p>
                    <p className="mt-2 font-semibold">{lead.recommended_property || "No automatic match"}{lead.recommended_room ? ` · ${lead.recommended_room}` : ""}</p>
                    {Number(lead.estimated_stay_total) > 0 && <p className="mt-1 text-sm text-gold">Estimated stay: USD {Number(lead.estimated_stay_total).toFixed(2)}</p>}
                    <p className="mt-2 text-xs text-gray-500">Re-check current availability, taxes and final selling price before quoting the customer.</p>
                  </div>
                </div>
