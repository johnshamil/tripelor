...
function unpack(row:any):ManagedProperty{return {...row.data,seasonalRates:Array.isArray(row.data?.seasonalRates)?row.data.seasonalRates:[],inventoryRules:Array.isArray(row.data?.inventoryRules)?row.data.inventoryRules:[],id:row.id,slug:row.slug,status:row.status,updated_at:row.updated_at};}
